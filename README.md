use case у4казать какие fr соответсвует

архитектурно значимые use case

extend на диаграммах

use case выбирать технолоигческие

Таблицы с терминами - заменить на ссылку на глосарий

Indicators в рисках - каждый должен быть с числом

Основы программной инженерии - курс

Роли в продукту по RAP

External Interfaces (Внешние интерфейсы - преподаватели заказчик по лору произведения

3.3 Roles and Responsibilities (Роли и обязанности) - таблицу


4. Management Process (Процесс управления) - диаграмма ганта


начало разработки февраль месяц, конец разработки - декабрь месяц, расписать план на два семестра


Вехи - номер use case основные, с вложенными, указать номер use case


4.3.2 Schedule Control Plan (План управления расписанием)
Еженедельный review прогресса. Используется Linear для отслеживания задач. - ссылка на доску задач
лучше специализированное ПО, репозиторий с проектом, GitLab доска с проектом


4.3.4 Reporting Plan (План отчетности) - описать действительный процесс


4.5 Close-out Plan (План завершения проекта) - указать дату завершения проекта


3. Business Context (Бизнес-контекст) - расписать по пользователям, какие боли проект решает


4. Product Objectives (Цели продукта) - указать более конкретные цели



5. Financial Forecast (Финансовый прогноз) - валюта из мира, график окупаемости, ROI


6. Constraints (Ограничения) - в рамках бизнес-контекста


Глосарий

роли и пользователя отдельно
...
например при снижении качества кода в каком-то количестве указать значение опасности риска



риски поделить по разным типа - риски разрабтки, политические

количество рисков - 8-10


риски должны относится к разработке
