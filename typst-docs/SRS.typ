...
#figure(
  ...
  caption: [Диаграмма 1 — UC-01: Работа с порталом Meth. Актор: Meth. Прецеденты: просмотр каталога тел, заказ свободного тела, предзаказ клонированного, просмотр статуса, скачивание сертификата.]
)

#figure(
  image("../use-case-diagrams/uc02-sleeve-catalog.png", width: 80%),
  caption: [Диаграмма 2 — UC-02: Управление каталогом тел. Актор: Sleeve Broker. Прецеденты: добавление тела, редактирование параметров, удаление тела.]
)

#figure(
  image("../use-case-diagrams/uc03-needlecast-procedure.png", width: 80%),
  caption: [Диаграмма 3 — UC-03: Проведение процедуры переноса. Актор: Needlecaster. Прецеденты: начало переноса, отметка успеха, отметка неудачи.]
)

#figure(
  image("../use-case-diagrams/uc04-validation.png", width: 80%),
  caption: [Диаграмма 4 — UC-04: Валидация и сертификация. Акторы: Psychosurgeon, Meth. Прецеденты: осмотр клиента, подтверждение, фиксация осложнений.]
)

#figure(
  image("../use-case-diagrams/uc05-admin.png", width: 80%),
  caption: [Диаграмма 5 — UC-05: Администрирование системы. Актор: Администратор. Прецеденты: создание учётной записи, блокировка/разблокировка, смена пароля.]
)
