...
#doctable(
  (table-header[Группа пользователей], table-header[Уровень компетенции], table-header[Способ взаимодействия]),
  ([Meth (Клиент)], [Новичок (технически), требовательный], [Веб-портал через защищённый браузер, минималистичный интерфейс, доступ только к своим данным.]),
  ([Sleeve Broker], [Опытный пользователь], [Десктоп-терминал с расширенными фильтрами, интеграция с внешними базами генетических архивов.]),
  ([Needlecaster], [Эксперт (технический)], [Специализированный интерфейс с таймерами, графиками в реальном времени, поддержкой стерильного ввода.]),
  ([Psychosurgeon], [Эксперт (медицинский)], [Интерфейс с медицинскими формами, доступ к ЭЭГ-данным, инструменты диагностики.]),
  ([Администратор / Директор], [Эксперт (системный)], [Панель аудита, доступ к неизменяемым логам, управление правами доступа.]),
)
...
#subsection("3.1.7")[FR-003-02: Блокировка повторного резервирования]

Система должна блокировать повторное резервирование одного sleeve.

#subsection("3.1.8")[FR-003-03: Авто-снятие резерва через 72 часа]

Система должна автоматически снимать резервирование через 72 часа, если sleeve не перешёл в статус «в подготовке».

#subsection("3.1.9")[FR-004-01: Хранение медицинской карты sleeve]
...
#subsection("3.1.12")[FR-005-02: Проверка конфликтов при назначении]

Система должна проверять конфликты при назначении (нельзя две операции в один слот).

#subsection("3.1.13")[FR-005-03: Уведомления персоналу за 24 часа]

Система должна отправлять уведомления ответственному персоналу за 24 часа до процедуры.

#subsection("3.1.14")[FR-006-01: Реестр кортикальных стеков]
...
#subsection("3.1.18")[FR-007-02: Алерт при превышении времени needlecast]

Система должна отправлять alert руководству, если процедура длится дольше 6 часов.

#subsection("3.1.19")[FR-008-01: Фиксация результатов пробуждения]
...
#subsection("3.1.22")[FR-009-02: Напоминания о дедлайнах валидации]

Система должна отправлять напоминания Psychosurgeon при приближении дедлайнов.

#subsection("3.1.23")[FR-010-01: Генерация сертификата совместимости в PDF]
...
#subsection("3.1.38")[FR-018-01: Система уведомлений]

Система должна отправлять уведомления пользователям при наступлении событий: изменение статуса кейса (для Meth), назначение процедуры (для персонала), алерт при отклонении параметров (для Needlecaster и Psychosurgeon), приближение дедлайна валидации (для Psychosurgeon). Способ доставки уведомлений: внутрисистемное (bell icon в интерфейсе) и email-дублирование.

#subsection("3.1.39")[FR-018-02: Настройка уведомлений пользователем]
...
#subsection("3.1.40")[FR-019-01: Управление контрактами с клиентами]

Система должна позволять администратору создавать и хранить контракты с клиентами (Meth): тип услуги, стоимость, дата заключения, статус оплаты, прикреплённые сканы подписанных документов.

#subsection("3.1.41")[FR-019-02: Отслеживание оплат]
...
#subsection("3.1.46")[FR-022-01: Журнал действий пользователей]

Система должна вести журнал действий всех пользователей: кто, когда, какое действие выполнил (создание/изменение/удаление сущности), с какого IP-адреса. Журнал доступен только администратору и не подлежит редактированию.

#subsection("3.1.47")[FR-022-02: Экспорт данных]
...
#subsection("3.2.1")[Время обучения]

#doctable(
  (table-header[Группа пользователей], table-header[Время на освоение], table-header[Конкретные требования к интерфейсу]),
  ([Новый персонал (Broker, Needlecaster, Psychosurgeon)], [<= 4 часов на освоение базового функционала], [Пошаговые мастера для критических операций (резервирование, needlecast); Контекстные подсказки при первом запуске модуля; Доступ к базе знаний из любого экрана (горячая клавиша F1).]),
  ([Meth (Клиент)], [<= 5 минут на первое использование], [Навигация: не более 3 кликов до любой ключевой функции (статус, документы, контакты); Визуальная индикация: цветовая схема статусов процедуры (ожидание → в процессе → завершено); Формы: автозаполнение известных данных, валидация полей в реальном времени; Помощь: всплывающие tooltips при наведении на неочевидные элементы; Документы: кнопка «Скачать сертификат» видна на дашборде без перехода в подменю.]),
)

#subsection("3.2.2")[Время выполнения типовых задач]

- Резервирование sleeve: <= 2 минуты.
- Регистрация этапа needlecast: <= 30 секунд.
- Генерация сертификата: <= 10 секунд.
- Поиск sleeve по параметрам: <= 3 секунды при базе до 10 000 записей.

#subsection("3.2.3")[Доступность интерфейса]
...
- Автоматическое резервное копирование критических данных каждые 5 минут.
...
- Обязательное code review для всех изменений, затрагивающих модули работы со стеками.
...
- Запрет на использование open-source компонентов с лицензиями, требующими раскрытия исходного кода продукта.
