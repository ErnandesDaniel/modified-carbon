...
*Диаграмма 2 — UC-02 ManageSleeveReserve (Пополнение резерва тел). Акторы: Sleeve Broker, Генетический архив. Прецеденты: заказ культивирования, приёмка тела.*

![UC-03: Проведение переноса](../use-case-diagrams/uc03-ConductNeedlecast.png)

*Диаграмма 3 — UC-03 ConductNeedlecast (Проведение переноса). Акторы: Needlecaster, Meth. Прецеденты: начало переноса, отметка результата; ProcedureFailed (extend).*

![UC-04: Осмотр и сертификация](../use-case-diagrams/uc04-ExamineAndCertify.png)

*Диаграмма 4 — UC-04 ExamineAndCertify (Осмотр и сертификация). Акторы: Psychosurgeon, Meth. Прецеденты: осмотр клиента, генерация сертификата; Complications (extend).*

![UC-05: Регистрация клиента](../use-case-diagrams/uc05-RegisterMeth.png)

*Диаграмма 5 — UC-05 RegisterMeth (Регистрация клиента). Акторы: Meth (новый клиент), Google OAuth. Прецеденты: авторизация через Google, создание учётной записи; AccountExists (extend).*
...
7. Постпроцедурное наблюдение: осмотр после пробуждения, контрольные тесты, фиксация диагноза.
...
|---------------------|---------------------|------------------------------|
| Meth (Клиент) | Новичок (технически), требовательный | Личный кабинет: каталог доступных тел с фильтрами и параметрами, оформление заказа тела (доступное или предзаказ клонированного), отслеживание прогресса культивирования (процент готовности, этап, дата готовности), статус кейса (таймлайн), скачивание сертификатов и документов, история переносов, уведомления. |
| Sleeve Broker | Опытный пользователь | Каталог sleeves с фильтрацией, заказ культивирования в генетическом архиве, приёмка поступивших тел, установка и активация в хранилище, резервирование за клиентом, подтверждение заказов, календарь культивации и резервирования. |
| Needlecaster | Эксперт (технический) | Журнал процедур с пошаговыми чеклистами, отслеживание статуса передачи DHF в реальном времени, форма фиксации результатов этапов, дашборд активной процедуры, регистрация инцидентов. |
| Psychosurgeon | Эксперт (медицинский) | Проведение осмотра после пробуждения (контрольные вопросы, когнитивные тесты), фиксация результатов на контрольных точках (24ч/48ч/72ч), регистрация диагноза, подтверждение сертификации, генерация сертификата совместимости. |
| Администратор / Директор | Эксперт (системный) | Панель аудита и логов, управление пользователями и ролями (RBAC), отчётность и экспорт, настройки системы, мониторинг безопасности. |
...
- Архитектурные: Клиент-серверная модель; сервер размещается на выделенном облачном VPS в изолированном защищённом сегменте.
...
### 3.1.8 FR-004-02: Ограничение доступа к медицинской карте

Система должна ограничивать доступ к медицинской карте только для Sleeve Broker и назначенного врача.

### 3.1.9 FR-005-01: Календарь процедур

Система должна отображать календарь операций с временными слотами.

### 3.1.10 FR-006-01: Реестр кортикальных стеков

Система должна вести учёт всех стеков: ID, владелец, местоположение, дата последнего извлечения, статус.

### 3.1.11 FR-006-02: История перемещений стеков

Система должна фиксировать историю перемещений: кто, когда, из пункта A в пункт B.

### 3.1.12 FR-007-01: Фиксация процедуры needlecast

Система должна фиксировать процедуру needlecast: время начала, статус передачи DHF (в процессе / завершено / ошибка), время завершения, ответственный Needlecaster, результат (успех / инцидент).

### 3.1.13 FR-008-01: Фиксация результатов пробуждения
...
### 3.1.14 FR-008-02: Блокировка перехода при непрохождении тестов

Система должна блокировать переход к валидации при непрохождении тестов.

### 3.1.15 FR-009-01: Фиксация результатов постпроцедурного наблюдения

Система должна позволять Psychosurgeon фиксировать результаты осмотра после пробуждения клиента: когнитивные тесты, показатели стабильности, диагноз (без осложнений / stack shock / фрагментация личности / отторжение тела).

### 3.1.16 FR-010-01: Генерация сертификата совместимости в PDF

Система должна автоматически генерировать сертификат совместимости в формате PDF при успешном прохождении постпроцедурного наблюдения.

### 3.1.17 FR-010-02: Состав сертификата совместимости

Сертификат должен содержать: реквизиты клиники, ID клиента, даты процедур, QR-код для проверки подлинности.

### 3.1.18 FR-011-01: Фиксация диагноза проблем переноса

Система должна позволять Psychosurgeon фиксировать диагноз (stack shock, диссонанс, отторжение).

### 3.1.19 FR-011-02: Создание инцидента и блокировка выпуска клиента

Система должна автоматически создавать инцидент с рекомендациями и блокировать выпуск клиента до разрешения проблемы.

### 3.1.20 FR-012-01: Неизменяемый аудит операций со стеками

Система должна вести неизменяемый журнал (audit log) всех операций со стеками, доступный только директору.

### 3.1.21 FR-013-01: Доступ Meth к информации в личном кабинете

Система должна предоставлять Meth доступ к порталу: просмотр каталога доступных тел с фильтрами и параметрами (рост, вес, генетический профиль, возраст, пол), оформление заказа тела (доступное или предзаказ клонированного), отслеживание прогресса культивирования (процент готовности, этап, плановая дата готовности), текущий статус кейса, скачивание сертификатов и документов, история переносов, уведомления.

### 3.1.22 FR-013-02: Изоляция данных Meth в личном кабинете

Система должна обеспечивать полную изоляцию данных: Meth видит только свою информацию.

### 3.1.23 FR-014-01: Регистрация Meth на внешнем портале
...
### 3.1.24 FR-014-02: Аутентификация Meth на внешнем портале
...
### 3.1.25 FR-014-03: Управление профилем Meth
...
### 3.1.26 FR-015-01: Управление учётными записями персонала
...
### 3.1.27 FR-015-02: Ролевая модель доступа
...
### 3.1.28 FR-016-01: Дашборд администратора
...
### 3.1.30 FR-017-01: Поиск по системе
...
### 3.1.31 FR-017-02: Генерация отчётов
...
### 3.1.32 FR-018-01: Отправка уведомлений

Система должна автоматически отправлять уведомления пользователям по настроенным каналам (email, SMS, push) при наступлении событий: изменение статуса кейса, завершение культивирования, результат процедуры, инцидент.

### 3.1.33 FR-018-02: Настройка уведомлений пользователем
...
### 3.1.34 FR-019-01: Управление контрактами

Система должна позволять администратору создавать и редактировать контракты с клиентами (Meth): реквизиты сторон, условия оказания услуг, стоимость, сроки, статус подписания.

### 3.1.35 FR-019-02: Отслеживание оплат
...
### 3.1.36 FR-020-01: Онбординг нового клиента
...
### 3.1.37 FR-020-02: Медицинская анкета клиента
...
### 3.1.38 FR-021-01: Хранение документов
...
### 3.1.39 FR-021-02: Шаблоны документов
...
### 3.1.40 FR-022-01: Журнал действий пользователей

Система должна вести неизменяемый журнал (audit log) всех действий пользователей в системе: вход, просмотр, создание, изменение, удаление данных. Журнал содержит: timestamp, пользователя, действие, объект, старое и новое значение.

### 3.1.41 FR-022-02: Экспорт данных
...
- Личный кабинет Meth: дашборд со статусом кейса, датами процедуры, документами, контактами специалистов.
- Адаптивный дизайн для доступа с любого устройства (десктоп, планшет, нейроинтерфейс).

**Внутренний портал (Internal Portal)** — доступен только через защищённый VPN-канал клиники, предназначен для персонала:
...
Система является веб-приложением и не требует специальных аппаратных интерфейсов. Взаимодействие с оборудованием осуществляется через стандартные веб-протоколы:

- Считыватели биометрических ключей: подключение через USB, взаимодействие через WebUSB API (для аутентификации Meth).
- Медицинское оборудование (мониторинг стеков): данные передаются через REST API шлюза медицинских устройств (отдельная система, не входит в SCMS).
- Терминалы для жестового управления (Needlecaster): подключение через локальную сеть, взаимодействие через WebSocket.
...
- Изолированный сетевой сегмент (VPC) и защищённый VPN-канал для внутреннего трафика.
...
- Установка: выделенный облачный сервер (VPS) в защищённом сегменте провайдера; внутренний портал — доступ по защищённому VPN-каналу.
