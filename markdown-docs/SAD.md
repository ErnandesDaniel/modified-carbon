...
1. **Изоляция сети:** Внутренний портал работает только в изолированной сети клиники (intranet). Внешний портал доступен через защищённый HTTPS.
...
### UC-01: OrderSleeve (Заказ sleeve клиентом)

**Актор:** Meth. **Цель:** Просмотр каталога, заказ теля, отслеживание статуса, скачивание сертификата.
...
- Модуль `validation` — генерация PDF-сертификата после успешной валидации.

### UC-02: ManageSleeveLifecycle (Управление жизненным циклом sleeve)

**Актор:** Sleeve Broker. **Цель:** Заказ культивирования, приёмка тела, резервирование за клиентом.
...
- Модуль `notification` — уведомления Psychosurgeon и администратора при завершении/инциденте.

### UC-04: ValidateAndCertify (Валидация и сертификация)
...
- Модуль `notification` — уведомление Sleeve Broker о новом клиенте.
